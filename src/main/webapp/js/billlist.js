...
function deleteBill(obj){
	$.ajax({
		type:"GET",
		url:path+"/sys/deleteBill/"+obj.attr("billid"),
		// data:{"bill.id":obj.attr("billid")},
		dataType:"json",
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/sys/viewBill/"+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/sys/modifyBeforeBill/"+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
