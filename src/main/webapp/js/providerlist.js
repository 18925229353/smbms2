...
function deleteProvider(obj){
	$.ajax({
		...
		// data:{"provider.id":obj.attr("proid")},
		dataType:"json",
		success:function(data){
			if(data.result == "true"){//删除成功：移除删除行
			/*	cancleBtn();*/
				alert("删除供应商【"+obj.attr("proname")+"】成功!");
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/sys/viewProvider/"+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/sys/modifyBeforeProvider/"+ obj.attr("proid");
	});
	...
	$(".deleteProvider").on("click",function(){
		...
		if(confirm("你确定要删除供应商【"+obj.attr("proname")+"】吗？")){
			$.ajax({
				...
				data:{"smbmsProvider.id":obj.attr("proid")},
				dataType:"json",
				success:function(data){
